...
document.querySelector("#save").addEventListener("click", () => {
  ...
  localStorage.setItem(
    "DARK_MODE",
    document.querySelector("#dark-mode-toggle").checked
  );
  ...
  //SET DEFAULT PROJECT
  let newDefaultProject = document.querySelector("#select-default-project")
    .value;
  let newDefaultProjectLabel = document.querySelector("#select-default-project")
    .options[document.querySelector("#select-default-project").selectedIndex]
    .label;
  let newDefaultProjectImage =
    "assets/images/thumbnails/" +
    document
      .querySelector("#select-default-project")
      .options[
        document.querySelector("#select-default-project").selectedIndex
      ].label.toLowerCase() +
    ".png";
  ...
  updateDefaultProject(
    newDefaultProject,
    newDefaultProjectLabel,
    newDefaultProjectImage
  );
...
document.querySelector("#reset").addEventListener("click", () => {
  ...
  updateDefaultProject(
    "projects/graphics/js_canvas/flowing_squares/index.html",
    "FLOWING SQUARES",
    "assets/images/thumbnails/flowing squares.png"
  );
...
function toggleDarkMode(state) {
  if (state === true || state === "true") {
    ...
    (defaultLinkColor = "rgb(30, 30, 30)"), (logoHoverColor = "black");
  }
  ...
  document.querySelectorAll(".mobile-link a")[
    activeLink
  ].style.color = UI_COLOR;
}
...
function updateDefaultProject(project, label) {
  ...
  document.getElementById("project-title").innerHTML =
    label +
    `<a href="${project}", target="_blank", rel="noopener noreferrer"><img id="project-ext" src="assets/images/util/external.svg" alt="open in new tab"></a>`;
...
function updateUI(color) {
  ...
  logo.addEventListener("mouseover", e => {
    if (e.target === logo) e.target.style.backgroundColor = logoHoverColor;
  });
  logo.addEventListener("mouseout", e => {
    if (e.target === logo) e.target.style.backgroundColor = color;
  });
  let footerLogo = document.querySelector("#footer-logo");
  footerLogo.style.backgroundColor = color;
  footerLogo.addEventListener("mouseover", e => {
    if (e.target === footerLogo)
      e.target.style.backgroundColor = logoHoverColor;
  });
  footerLogo.addEventListener("mouseout", e => {
    if (e.target === footerLogo) e.target.style.backgroundColor = color;
  });
  ...
  [...document.querySelectorAll(".project .thumbnail img")].forEach(
    thumbnailOver
  );
  let thumbnailOut = e =>
  ...
  let linkClick = e =>
    e.addEventListener("click", e => {
      ...
      document.querySelectorAll(
        ".mobile-link a"
      )[0].style.color = defaultLinkColor;
      document.querySelectorAll(
        ".mobile-link a"
      )[1].style.color = defaultLinkColor;
      document.querySelectorAll(
        ".mobile-link a"
      )[2].style.color = defaultLinkColor;
      if (target == 0) {
  ...
  let linkOver = e =>
    e.addEventListener("mouseover", e => {
      if (e.target.getAttribute("data-show") != activeLink)
  ...
  e.addEventListener("click", e => {
    if (e.target.nodeName.toLowerCase() === "img") {
      ...
      document.getElementById("project-title").innerHTML =
        ...
        `<a href="${src}", target="_blank", rel="noopener noreferrer"><img id="project-ext" src="assets/images/util/external.svg" alt="open in new tab"></a>`;
      window.scrollTo(0, 0);
    }
  });
[
  ...document.querySelectorAll("#projects .category .grid .project .thumbnail")
].forEach(projectClick);
